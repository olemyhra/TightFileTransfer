...
int main(void)
{
	...
	if (recieve_buffer == NULL) {
		fprintf(stderr, "Unable to allocate memeory to recieve buffer!\n");
		exit(EXIT_FAILURE);
	}
	...
	if (bind(server_socket, (struct sockaddr *) &server_address, sizeof(server_address)) < 0) {		fprintf(stderr, "Unable to bind address to socket!\n");
		exit(EXIT_FAILURE);
	}
